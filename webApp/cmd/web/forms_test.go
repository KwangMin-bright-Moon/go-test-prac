...
func TestForm_Required(t *testing.T){
	...
	if form.Valid() {
		t.Error("form shows vaild when required fields are missing")
	}
...
func TestForm_Check(t *testing.T){
	...
	if form.Valid() {
		t.Errorf("Vaild() return false, and it should be true when calling Check()")
	}
}
...
func TestForm_ErrorGet(t *testing.T){
	...
	if len(s) == 0 {
		t.Errorf("should have an error returned from Get, but do not")
	}
